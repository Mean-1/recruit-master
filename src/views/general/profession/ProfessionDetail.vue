      ...
      <div class="detail-header">
        <div class="job-box margin-20per">
          <div class="job-description">
            ...
            <div class="job-tag"><span v-for="(tag,index) in jobData.tag" :key="tag + index">{{ tag }}</span></div>
          ...
          <div class="similar-position">
            ...
            <ul>
              <li v-for="(sim, index) in similarJob" :key="sim.company_name + index">
                ...
                <el-link class="sim-job-info" :title="sim.job_duty" :href="'/profession/detail?job_id=' + sim.job_id">
                  <span>{{ sim.job_duty }}</span>
                </el-link>
                <p class="sim-job-salary">{{ sim.job_salary }}</p>
                <div class="sim-company-info">
                  <el-link class="sim-job-company" :title="sim.company_name" :href="'/company/detail?company_id=' + sim.company_id">{{ sim.company_name }}</el-link>
                  <span class="office-city">&nbsp;&nbsp;&nbsp;·&nbsp;&nbsp;&nbsp;{{ sim.office_city }}</span>
                </div>
              </li>
            </ul>
            <el-link class="show-more" :href="'/profession'">查看更多职位<i class="el-icon-arrow-right"></i></el-link>
          </div>
        </aside>
          ...
          <div class="job-recruiter">
            ...
            <div class="content">
              ...
              <div class="recruiter">
                ...
                <p>
                  <span>{{ jobData.interviewerDuty }}</span>
                  <el-divider direction="vertical"></el-divider>
                  <template v-if="jobData.update_date">
...
export default {
  ...
  components: { GeneralTopBar, GeneralFooter, Map},
    data() {
        return {
            // jobData: {},
            jobData: {
    ...
    methods: {
        async initData() {
            //这里不会String转int
            // if(null!=this.$route.query.job_id)  {
            //   this.id =parseInt(this.$route.query.job_id);
            // }
            ...
            // if(res.msg === 'success'){
              ...
              res1.forEach(item => {
                if(item.company.icon!=null){
                  // let str = "@/image/company/"+item.company.icon;
            ...
            this.similarJob = Object.assign([],this.similarJob, res1);
        ...
        async judgeCollect() {
          // 判断是否已收藏
          const res = await this.$axios.request({
            url: `/user-job-collection/isCollection/${JSON.parse(window.sessionStorage.getItem('user')).id}/${this.$route.query.job_id}`,
            method: "get",
          })
          console.log(res);
        ...
        async collect() {
          console.log(JSON.parse(window.sessionStorage.getItem('user')).is_recruit)
            ...
            if(JSON.parse(window.sessionStorage.getItem('user')).is_recruit !== "0") {
              ...
              if(!this.isCollect){
                const res = await this.$axios.request({
                  url: `/user-job-collection/addCollection/${JSON.parse(window.sessionStorage.getItem('user')).id}/${this.$route.query.job_id}`,
                  method: "put",
                })
                console.log(res);
                if("success"==res.message){
                ...
                const res = await this.$axios.request({
                  url: `/user-job-collection/deleteCollection/${JSON.parse(window.sessionStorage.getItem('user')).id}/${this.$route.query.job_id}`,
                  method: "delete",
                })
                console.log(res);
                if("success"==res.message){
                  ...
                  return this.$message.error("收藏取消失败");
                }
        ...
        async judgeApply() {
            // 判断是否已投递
            const res = await this.$axios.request({
                url: `/user-job-applycation/isApply/${JSON.parse(window.sessionStorage.getItem('user')).id}/${this.$route.query.job_id}`,
                method: "get",
            })
            console.log(res);
            ...
            if(!JSON.parse(window.sessionStorage.getItem('user')).is_recruit){
                ...
                if(!this.isApply) {
                    const validReusme = await this.$axios.request({
                        url: `/user-job-applycation/addApply/${JSON.parse(window.sessionStorage.getItem('user')).id}/${this.$route.query.job_id}`,
                        method: "put",
                    })
                    ...
                    // })
                    // console.log(res)
                  if("success"==validReusme.message){
                    ...
                    const res = await this.$axios.request({
                        url: `/user-job-applycation/deleteApply/${JSON.parse(window.sessionStorage.getItem('user')).id}/${this.$route.query.job_id}`,
                        method: "delete",
                    })
                    console.log(res)
                    if("success"==res.message ) {
