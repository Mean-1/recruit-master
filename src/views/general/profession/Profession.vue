...
<!--              <SelectWrapper @getOption="search1"></SelectWrapper>-->
<!--              getOption($event)-->
        ...
        <!-- 职位信息 -->
        <div class="job-box margin-10per">
          <!-- 右侧侧栏 -->
          <aside>
            <!-- 登录盒子 -->
              ...
              <li v-for="(job, jobIndex) in jobList" :key="job.name + jobIndex">
                <div class="job-wrapper">
                  <div class="info-primary">
                    <el-link :href="'/profession/detail?job_id=' + job.id" class="job-info">
                      <div class="job-title">
                        <span>{{ job.name }}</span>
...
export default {
    ...
    data() {
        return {
            ...
            city:"",
            //搜索框的值
            searchKey: this.$route.params.job,
            currentOption:"",
            //公司地址,城市
            ...
            filterConditions: [
                ...
                {
                    label: "学历水平", options: ["不限","中专","高中","大专","本科","硕士","博士"]
                },
                {
    ...
    created() {
        this.initData();
      console.log("liuchen",this.$route.params.job)
    },
    methods: {
        initData() {
          ...
          console.log(this.searchKey)
            let getJobList = async () => {
                ...
                res.data.forEach(item => {
                  ...
                  item.interviewer=item.users.username;
                  item.address=item.company.address;
                  item.companyName=item.company.name;
                  item.companySize=item.company.size;
                  item.companyTag = item.company.tag;
                  item.companyBenefit=item.company.benefit;
        ...
        search(){
          ...
          for(i;i<user.label.length;i++){
            if(user.label[i]=="工作经验") this.experience=user.option[i];
            else if(user.label[i]=="学历水平") this.qualification=user.option[i];
            else if(user.label[i]=="薪资要求") this.salary=user.option[i];
            else if(user.label[i]=="公司规模") this.size=user.option[i];
          }
          if(this.conditionCity!="全国") this.city=this.conditionCity;
          if(this.experience=="不限") this.experience="";
          if(this.qualification=="不限") this.qualification="";
          if(this.salary=="不限") this.salary="";
          if(this.size=="不限") this.size="";
          if(this.conditionCity=="全国") this.city="";

          console.log(this.searchKey)
          console.log(this.conditionCity)
