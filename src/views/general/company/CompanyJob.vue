            ...
            <div class="company-primary">
              ...
              <div class="company-info">
                <div class="info-wrapper">
                  <el-link :href="'/company/job?company_id=' + company.company_id" :class="{'is-active': $route.path === '/company/job'}">{{ company.job_total }}个</el-link>
                  <p>招聘职位</p>
                </div>
<!--                <div class="info-wrapper">-->
            ...
            <ul>
              <li v-for="(rej,index) in recruit_job" :key="rej.job_duty + index">
                <el-link class="job-duty" :title="rej.job_duty"
                         :href="'/profession/detail?job_id=' + rej.job_id"> {{ rej.job_duty }}</el-link>
        ...
        return {
          ...
          searchKey:"",//搜索框的值
            ...
            filterConditions: [
                ...
                {
                    label: "学历要求", options: ["不限","中专","高中","大专","本科","硕士","博士"]
                },
                {
    ...
    methods: {
        ...
        async getCompany() {
            const res = await this.$axios.request({
                url: `/company/getCompanyById/${this.$route.query.company_id}`,
                method: "get",
            });
            console.log(res);
            // if(res.msg === 'success'){
            res.company_id=res.id;
            res.company_logo = require("@/image/company/" + res.icon);
            res.company_full_name=res.name;
            res.company_description=res.description;
            res.company_address=res.address;
            res.company_tag=res.tag;
            res.company_type=res.type;
            res.company_size=res.size;
            res.company_website=res.website;
            res.company_introduction=res.introduction;
            res.job_total=res.nums;
            this.company = Object.assign({},{},res);
            // }
        },
        async getFilterJob() {
          let i =0;
          //对筛选条件进一步赋值，以符合后端参数形式
          for(i;i<user.label.length;i++){
            if(user.label[i]=="经验要求") this.experience=user.option[i];
            else if(user.label[i]=="学历要求") this.qualification=user.option[i];
            else if(user.label[i]=="薪资待遇") this.salary=user.option[i];
            else if(user.label[i]=="工作城市") this.address=user.option[i];
          }
          if(this.experience=="不限") this.experience="";
          if(this.qualification=="不限") this.qualification="";
          if(this.salary=="不限") this.salary="";
          if(this.address=="不限") this.address="";
          console.log(this.salary)
          console.log(this.experience)
          console.log(this.qualification)
          console.log(this.address)
            const res = await this.$axios.request({
                url: `/job/getJobByPageWithCompanyId`,
                method: "get",
                params: {
                    currentPage: this.currentPage,
                    pageSize: this.pageSize,
                    cid:this.$route.query.company_id,
            ...
            // if(res.msg === 'success'){
                ...
                this.filterJob = Object.assign([],[],res.data);
            // }
        },
        async getRecruitJob() {
            const res = await this.$axios.request({
                url: `/job/getJobByCompanyId/${this.$route.query.company_id}`,
                method: "get",
            });
            console.log(res);
            // if(res.msg === 'success'){
