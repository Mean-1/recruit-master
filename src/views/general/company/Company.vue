            ...
            companyList: [
                {
                    ...
                    company_description: "做全球领先的重度游戏开发商"
                }
            //     {
    ...
    methods: {
        async initData() {
          // if (location.href.indexOf ("#reloaded")==-1) { location.href=location.href+"#reloaded"; location.reload (); }
            if(this.conditionType=="不限") this.conditionType="";
            if(this.conditionTag=="不限") this.conditionTag="";
            if(this.conditionSize=="不限") this.conditionSize="";
            if(this.conditionCity=="全国") this.conditionCity="";
          console.log(this.conditionCity)
          console.log(this.conditionSize)
          console.log(this.conditionTag)
          console.log(this.conditionType)
            ...
            const res = await this.$axios.request({
                ...
                params: {
                    ...
                    address: this.conditionCity,
                    tag:this.conditionTag,
                    type:this.conditionType,
                    c_size:this.conditionSize
                }
            });
            console.log(res);
            ...
            res.data.forEach(item => {
                ...
                item.company_logo = require("@/image/company/" + item.icon);
                item.company_name = item.name;
                item.company_tag  = item.tag;
        ...
        async conditionSelect(key, condition) {
            switch(key){
          ...
          this.companyList = Object.assign([], [], res.data);
            ...
            //加下面这个之后，会每次触发这个事件后，又重新调用initData()方法，就相当于查询所有，没有进行选择查询
