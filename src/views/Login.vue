...
<!--              </el-form-item>-->
              <el-form-item>
                <el-button type="primary" @click="submitForm('passwordForm')">注册/登录</el-button>
              </el-form-item>
            </el-form>
          </div>
            ...
            <dl>
              <dd>
                <h2 v-for="toggle in toggleLogin" :key="toggle.name"
                    ...
                    @click="toggleLoginType(toggle)">{{ toggle.name }}
                </h2>
              </dd>
            </dl>
          </div>
...
export default {
    ...
    data() {
        ...
        return {
            ...
            //登录方式保存
            loginType: Constant.LOGIN_PASSWORD_CODE,
    ...
    methods: {
      ...
      checkUserType() {
        ...
        changLogin(userType) {
            ...
            this.getValidateCode();
            this.loginType = Constant.LOGIN_PASSWORD_CODE;
        },
        // 获取随机4位验证码
        getValidateCode() {
            ...
            this.$refs[formName].validate(async (valid) => {
                if (valid) {
                    ...
                    //真正的登录位置
                      ...
                      console.log(typeof(this.passwordForm.phone))
                      console.log(typeof(this.passwordForm.password))
                      ...
                      const res = await this.$axios.request({
                        ...
                        if(login_role==""){
                          if (res.message === 'success') {
                            ...
                            // window.localStorage.setItem("login_role", user.is_recruit);
                        ...
                        }else if(login_role=="1"||login_role=="0"){
                          if(res.message==='success'){
                            console.log(res.data);
                    ...
                    this.$message.success("登录或注册成功");
                    await this.$router.push("/home");
                } else {
                    // alert("登录失败");
                    this.$message.error("登录或注册信息有误，请重新输入");
                    this.resetForm();
                }
            })
        },
