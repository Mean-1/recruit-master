  ...
  <header>
    ...
    <div class="top-bar">
      <nav class="margin-20per">
        ...
        <!-- 用户操作: 未登录状态-->
        <div v-if="!login_role" class="user-nav">
        ...
        <!-- 用户操作: 求职者身份-->
        <div v-if="login_role == 0" class="user-applicant">
          ...
          <el-dropdown>
            <span class="el-dropdown-link">
              欢迎回来，{{ name }}<i class="el-icon-arrow-down"></i>
            </span>
...
<!--              <el-dropdown-item><el-link @click="redirect('/applicant/interview')">我的面试</el-link></el-dropdown-item>-->
        ...
        <!-- 用户操作: 招聘官身份-->
        <div v-if="login_role == 1" class="user-recruiter">
...
export default {
    ...
    data() {
      // console.log(user.is_recruit+"//");

      return {

            name:JSON.parse(window.sessionStorage.getItem('user')) ? JSON.parse(window.sessionStorage.getItem('user')).username :"guest",
            login_role: JSON.parse(window.sessionStorage.getItem('user')) ? JSON.parse(window.sessionStorage.getItem('user')).is_recruit :null,
            dialogVisible: false,
